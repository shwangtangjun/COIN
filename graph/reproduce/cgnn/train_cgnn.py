...
from utils_cgnn import load_data, accuracy, get_best_params
...
def run_single_trial_of_single_split(adj, features, labels, idx_train, idx_val, idx_test, torch_seeds, opt):
    ...
    if opt['optimizer'] == 'adam':
        ...
        optimizer = optim.RMSprop(model.parameters(), lr=opt['lr'], weight_decay=opt['decay'])
    else:
...
def run_single_split(seed):
    ...
    adj, features, labels, idx_train, idx_val, idx_test = load_data(args.dataset, random_state, opt['self_loop_weight'])
