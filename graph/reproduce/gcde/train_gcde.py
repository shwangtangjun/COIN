...
from utils_gcde import load_data, accuracy, get_best_params
from gcde import GCDE
import copy
import time
import dgl
import os
...
def run_single_trial_of_single_split(adj, features, labels, idx_train, idx_val, idx_test, torch_seeds, opt):
    ...
    g = dgl.graph((edge_index[0], edge_index[1]), num_nodes=features.shape[0], device=device)
    ...
    model = GCDE(g, in_features=features.shape[1], out_features=labels.max().item() + 1)
