...
from utils_diffresnet import load_data, accuracy, get_best_params
from diff_resnet import DiffusionNet
import copy
import time
...
def run_single_trial_of_single_split(adj, features, labels, idx_train, idx_val, idx_test, diagonal,
                                     torch_seeds, opt):
    torch.manual_seed(torch_seeds)
    torch.cuda.manual_seed(torch_seeds)

    model = DiffusionNet(n_features=features.shape[1], num_classes=labels.max().item() + 1,
                         step=opt['step_size'],
                         layer_num=opt['layer_num'], dropout=opt['dropout'], diagonal=diagonal.cuda())
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.verbose:
            test_acc = test(model, adj, features, labels, idx_test)
