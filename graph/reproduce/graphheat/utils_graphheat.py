...
def wavelet_origin(adj, threshold, s):
    laplacian = sp.eye(adj.shape[0]) - normalize_adj(adj, normalization="symmetric")
    eigvals, eigenvecs = np.linalg.eigh(laplacian.toarray())

    eigvals_mod = np.exp(-eigvals * s)
    weight = eigenvecs @ np.diag(eigvals_mod) @ eigenvecs.T
    weight[weight < threshold] = 0

    weight = sp.csr_matrix(weight)
    weight.setdiag(0.)
    weight = sparse_mx_to_torch_sparse_tensor(weight)
    return weight
