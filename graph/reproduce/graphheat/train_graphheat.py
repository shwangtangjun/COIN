...
from utils_graphheat import load_data, accuracy, wavelet_origin, get_best_params
from gcn import GCN
import copy
import time
...
def run_single_trial_of_single_split(rewired_matrix, features, labels, idx_train, idx_val, idx_test, diagonal,
                                     torch_seeds):
    torch.manual_seed(torch_seeds)
    torch.cuda.manual_seed(torch_seeds)

    features = features.cuda()
    rewired_matrix = rewired_matrix.cuda()
    labels = labels.cuda()
    idx_train = idx_train.cuda()
    idx_val = idx_val.cuda()
    idx_test = idx_test.cuda()

    model = GCN(n_features=features.shape[1], n_hidden=16, num_classes=labels.max().item() + 1, dropout=0.5)
    model = model.cuda()
    ...
    for epoch in range(args.max_epochs):
        ...
        train(model, optimizer, rewired_matrix, features, labels, idx_train)

        val_loss, val_acc = val(model, rewired_matrix, features, labels, idx_val)
        ...
        if args.verbose:
            test_acc = test(model, rewired_matrix, features, labels, idx_test)
    ...
    model.load_state_dict(best_state_dict)
    acc = test(model, rewired_matrix, features, labels, idx_test)
    acc = acc.cpu().numpy()
...
def run_single_split(seed):
    ...
    threshold, s = get_best_params(args.dataset)
    ...
    if not os.path.isfile('rewired_adj_matrix/' + args.dataset + '.pt'):
        rewired_matrix = wavelet_origin(adj, threshold=threshold, s=s)
        torch.save(rewired_matrix, 'rewired_adj_matrix/' + args.dataset + '.pt')
    else:
        rewired_matrix = torch.load('rewired_adj_matrix/' + args.dataset + '.pt')

    torch_seeds = random_state.randint(0, 1000000, args.num_inits)  # 20 trials for each split
    acc_list = []
    for i in range(args.num_inits):
        acc = run_single_trial_of_single_split(rewired_matrix, features, labels, idx_train, idx_val, idx_test, diagonal,
                                               torch_seeds[i])
        acc_list.append(acc)
