...
from utils_appnp import load_data, accuracy
from appnp import PPNP
from propogation import PPRPowerIteration
import copy
import time
...
def run_single_trial_of_single_split(adj, features, labels, idx_train, idx_val, idx_test, torch_seeds):
    ...
    model = PPNP(nfeatures=features.shape[1], nclasses=labels.max().item() + 1, hiddenunits=[64], drop_prob=0.5,
                 propagation=PPRPowerIteration(adj, alpha=0.1, niter=10))
    model = model.to(device)

    optimizer = optim.Adam([{'params': model.non_reg_params, 'weight_decay': 0},
                            {'params': model.reg_params, 'weight_decay': 5e-3}], lr=0.01)
