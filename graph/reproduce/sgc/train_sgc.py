...
from utils_sgc import load_data, accuracy, get_best_params
...
def run_single_trial_of_single_split(adj, features, labels, idx_train, idx_val, idx_test, torch_seeds, opt):
    ...
    features, precompute_time = sgc_precompute(features, adj)

    model = SGC(features.size(1), labels.max().item() + 1)
    model = model.to(device)

    optimizer = optim.Adam(model.parameters(), lr=0.2, weight_decay=opt['weight_decay'])
